...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const skolyarProiz = x1 * x2 + y1 * y2;
  const modulVector1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modulVector2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = skolyarProiz / (modulVector1 * modulVector2);
...
function roundToPowerOfTen(num, pow) {
  let x = num;
  if (pow !== 0) {
    x = Math.round(num / 10 ** pow) * 10 ** pow;
  }
  return x;
}
/**
